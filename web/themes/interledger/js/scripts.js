...
if (document.contains(siteNav)) {
  ...
  subLinks.forEach((subLink) => {
    subLink.setAttribute("aria-expanded", false);
    subLink.addEventListener("click", function handleSubLinkClick(event) {
      ...
      const notClickedLinks = allSubLinks.filter((otherLink) => {
  ...
  siteNav.addEventListener("keydown", function handleKeydown(event) {
    if (event.key === "Escape") {
